  ...
  const addIssue = async (issueData: any) => {
    try {
      const res = await fetch(`/api/issues`, {
        ...
        headers,
        body: JSON.stringify({
          title: `Auto Issue: ${issueData.description.slice(0, 40)}...`,
      ...
      if (!res.ok) {
        console.error("addIssue failed:", res.status, data);
      ...
      if (data?.predicted_category) {
        toast.success(`Issue reported • Category: ${data.predicted_category}`);
      } else {
